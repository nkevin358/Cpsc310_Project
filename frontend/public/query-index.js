...
function buttonClickHandler () {
    let queryObj = CampusExplorer.buildQuery(); //2
    CampusExplorer.sendQuery(queryObj).then((result) => { //3
        CampusExplorer.renderResult(JSON.parse(result));//4
    }).catch((err) => {
        // console.log(err);
     });
}
document.getElementById('submit-button').onclick = buttonClickHandler; //1
